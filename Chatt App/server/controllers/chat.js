import { ALERT, REFETCH_CHATS } from "../constants/eveents.js";
...
const leaveGroup = TryCatch(async (req, res, next) => {
  ...
  if (chat.creator.toString() !== req.user.toString()) {
    ...
    const newChatCreator = remainingMembers[randomElement];
    chat.creator = newChatCreator;
  }
...
const sendAttachments = TryCatch(async (req, res, next) => {
  ...
  if (files.length > 5)
    return next(new ErrorHandler("Files can not be more the 5", 400));
  ...
  if (files.length < 1)
    return next(new ErrorHandler("Please Provide Attachment", 404));
